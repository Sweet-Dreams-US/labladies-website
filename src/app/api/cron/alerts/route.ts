...
export async function GET(req: Request) {
  ...
  const alerts = [...computeAlerts(rows), ...computeInquiryAlerts(inquiries)];

  const already = new Set((await listSentAlerts()).map((a) => `${a.entity_id}:${a.rule}`));
  const fresh = alerts.filter((a) => !already.has(`${a.entityId}:${a.rule}`));
  ...
  const headings: Record<AlertRule, string> = {
    inquiry_uncontacted: "Website inquiries nobody has called back",
          ...
          .map(
            (a) => `<tr>
              <td style="padding:8px 12px 8px 0;vertical-align:top;border-top:1px solid #f6ead6">
                <strong style="color:#1a1512">${escapeHtml(a.subject)}</strong><br>
  ...
  if (result === "sent") {
    await Promise.all(fresh.map((a) => recordAlertSent(a.entityId, a.rule)));
